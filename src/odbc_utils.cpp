...
// void OdbcUtils::Check(int rc, sqlite3 *db) {
// 	// if (rc != SQLITE_OK) {
// 	// 	throw std::runtime_error(string(sqlite3_errmsg(db)));
// 	// }
// }

string OdbcUtils::TypeToString(int sqlite_type) {
	// switch (sqlite_type) {
	// case SQLITE_ANY:
	// 	return "any";
	// case SQLITE_INTEGER:
...
string OdbcUtils::SanitizeString(const string &table_name) {
	return StringUtil::Replace(table_name, "'", "''");
}

string OdbcUtils::SanitizeIdentifier(const string &table_name) {
	return StringUtil::Replace(table_name, "\"", "\"\"");
}

LogicalType OdbcUtils::ToOdbcType(const LogicalType &input) {
	switch (input.id()) {
...
LogicalType OdbcUtils::TypeToLogicalType(const string &sqlite_type) {
