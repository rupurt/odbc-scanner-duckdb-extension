...
public:
  ...
  void Tables(OdbcTableOptions opts) {
    ...
    auto sql_return = SQLTables(
        handle, (SQLCHAR *)opts.catalog_name.c_str(), SQL_NTS, (SQLCHAR *)opts.schema_name.c_str(),
    ...
    if (sql_return == SQL_SUCCESS || sql_return == SQL_SUCCESS_WITH_INFO) {
      ...
      auto diagnostics = ExtractDiagnostics(SQL_HANDLE_STMT, handle);
      throw Exception("OdbcStatement->Tables() SQLTables SQL_RETURN=" + std::to_string(sql_return) +
                      " SQL_STILL_EXECUTING msg='" + diagnostics->msg + "' state=" +
      ...
      auto diagnostics = ExtractDiagnostics(SQL_HANDLE_STMT, handle);
      throw Exception("OdbcStatement->Tables() SQLTables SQL_RETURN=" + std::to_string(sql_return) +
                      " SQL_INVALID_HANDLE msg='" + diagnostics->msg + "' state=" +
                      diagnostics->state + " native=" + std::to_string(diagnostics->native));
    } else {
      auto diagnostics = ExtractDiagnostics(SQL_HANDLE_STMT, handle);
      throw Exception("OdbcStatement->Tables() SQLTables SQL_RETURN=" + std::to_string(sql_return) +
                      " UNKNOWN_SQL_RETURN msg='" + diagnostics->msg + "' state=" +
                      diagnostics->state + " native=" + std::to_string(diagnostics->native));
    }
  }
  void GetDescriptorField() {
    SQLHDESC handle_descriptor;
    auto statement_attr_sql_return =
        SQLGetStmtAttr(handle, SQL_ATTR_APP_ROW_DESC, &handle_descriptor, 0, NULL);
    if (statement_attr_sql_return == SQL_SUCCESS ||
      ...
      auto diagnostics = ExtractDiagnostics(SQL_HANDLE_STMT, handle);
      throw Exception("OdbcStatement->GetDescriptorField() SQLGetStmtAttr SQL_RETURN=" +
      ...
      auto diagnostics = ExtractDiagnostics(SQL_HANDLE_STMT, handle);
      throw Exception("OdbcStatement->GetDescriptorField() SQLGetStmtAttr SQL_RETURN=" +
                      std::to_string(statement_attr_sql_return) + " SQL_INVALID_HANDLE msg='" +
                      diagnostics->msg + "' state=" + diagnostics->state +
                      " native=" + std::to_string(diagnostics->native));
    } else {
      auto diagnostics = ExtractDiagnostics(SQL_HANDLE_STMT, handle);
      throw Exception("OdbcStatement->GetDescriptorField() SQLGetStmtAttr SQL_RETURN=" +
                      std::to_string(statement_attr_sql_return) + " UNKNOWN_SQL_RETURN msg='" +
                      diagnostics->msg + "' state=" + diagnostics->state +
                      " native=" + std::to_string(diagnostics->native));
    }
    ...
    auto retcode =
        SQLGetDescField(handle_descriptor, 0 + 1, SQL_DESC_CONCISE_TYPE, &descConsiseType, 0, 0);
...
protected:
  ...
  void SqlDataTypeToCDataType(OdbcColumnDescription *col_desc) {
    ...
    case SQL_ARD_TYPE: {
      ...
      throw Exception("OdbcSqlDataTypeToCDataType() SQL_ARD_TYPE application row "
