...
static unique_ptr<FunctionData> AttachBind(ClientContext &context, TableFunctionBindInput &input,
  ...
  std::cout << "### OdbcAttach.AttachFunction - before statement->DescribeColumns()" << std::endl;
  // statement->GetDescriptorField();
  auto columns = statement->DescribeColumns();
  std::cout << "### OdbcAttach.AttachFunction - after statement->DescribeColumns()" << std::endl;
  ...
  do {
    ...
    for (auto r = 0; r < rows_fetched; r++) {
      ...
      if ((row_status == SQL_ROW_SUCCESS) || (row_status == SQL_ROW_SUCCESS_WITH_INFO)) {
        ...
        throw Exception("OdbcAttach() row status=" + std::to_string(row_status) + " SQL_ROW_PROCEED");
      } else if (row_status == SQL_ROW_IGNORE) {
        throw Exception("OdbcAttach() row status=" + std::to_string(row_status) + " SQL_ROW_IGNORE");
      } else {
        throw Exception("OdbcAttach() row status=" + std::to_string(row_status) + " SQL_ROW_UNKNOWN");
      }
      ...
      auto schema_catalog_entry = Catalog::GetSchema(context, "memory", schema, OnEntryNotFound::RETURN_NULL);
      if (schema_catalog_entry == nullptr) {
        std::cout << "schema not found so create it schema=" << schema << std::endl;
        auto &catalog = Catalog::GetCatalog(context, "memory");
        // auto create_schema_info = make_uniq<CreateSchemaInfo>();
        // create_schema_info->schema = schema;
        // auto catalog_entry = catalog.CreateSchema(context, std::move(create_schema_info));
        ...
        // MemorySchema info;
        // info.schema = schema;
        // auto &info = make_uniq<SchemaCatalogEntry>(catalog, schema, false);
        ...
        if (catalog_entry == nullptr) {
          std::cout << "schema NOT created..." << std::endl;
        } else {
          std::cout << "schema WAS created..." << std::endl;
          std::cout << "parent catalog " << catalog_entry->ParentCatalog().GetName() << std::endl;
          auto schema_catalog_entry_2 =
              Catalog::GetSchema(context, "memory", schema, OnEntryNotFound::RETURN_NULL);
          if (schema_catalog_entry == nullptr) {
            std::cout << "AFTER SCHEMA WAS CREATED GetSchema returned the schema! " << schema << std::endl;
          } else {
            std::cout << "AFTER SCHEMA WAS CREATED GetSchema returned nullptr... " << schema << std::endl;
          }
        }
      } else {
        std::cout << "schema found so use it schema=" << schema << std::endl;
      }
...
static void AttachFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
  std::cout << "### OdbcAttach.AttachFunction" << std::endl;
  ...
  do {
    ...
    for (auto r = 0; r < rows_fetched; r++) {
      ...
      // auto d2conn = Connection(context.db->GetDatabase(context));
      ...
      dconn
          ...
          ->CreateView(schema, table_name, data.overwrite, false);
